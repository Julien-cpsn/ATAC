...
impl App<'_> {
    pub(super) fn render_jwt_token_tab(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let payload_editor = self.auth_jwt_payload_text_area.multi_line_editor(
            Some("Payload"),
        ...
        frame.render_widget(&mut self.auth_jwt_secret_text_input, jwt_token_auth_layout[2]);
        frame.render_widget(payload_editor, jwt_token_auth_layout[3]);
    }
}

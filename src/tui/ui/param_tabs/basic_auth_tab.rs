...
impl App<'_> {
    pub(super) fn render_basic_auth_tab(&mut self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(&mut self.auth_basic_username_text_input, basic_auth_layout[0]);
        frame.render_widget(&mut self.auth_basic_password_text_input, basic_auth_layout[1]);
    }
}